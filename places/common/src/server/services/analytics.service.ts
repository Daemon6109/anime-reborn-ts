...
export class AnalyticsService implements OnInit, OnStart {
    ...
    private async handlePlayerJoined(player: Player): Promise<void> {
        ...
        this.trackEvent(player, "PlayerJoined", new Map<string, unknown>([
    ...
    private handlePlayerLeft(player: Player): void {
        ...
        if (sessionInfo) {
            ...
            this.trackEvent(player, "PlayerLeft", new Map<string, unknown>([
    ...
    private handleCharacterAdded(player: Player): void {
        ...
        this.trackEvent(player, "CharacterSpawned", new Map<string, unknown>([
            ["spawnTime", os.time()],
        ]));
    }
    ...
    public trackPerformance(player: Player): void {
        ...
        this.trackEvent(player, "PerformanceMetrics", new Map<string, unknown>([
            ["sessionDuration", sessionDuration],
    ...
    public getSessionData(player: Player): { joinTime?: number; lastActivity?: number; sessionDuration?: number } | undefined {
    ...
    private processPendingEvents(isShuttingDown = false): void {
        ...
        const eventsToProcess = isShuttingDown ? this.pendingEvents.size() : math.min(BATCH_SIZE, this.pendingEvents.size());
