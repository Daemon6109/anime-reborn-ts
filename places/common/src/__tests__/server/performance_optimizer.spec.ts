...
describe("PerformanceOptimizer", () => {
	...
	afterEach(() => {
		...
		const optimizerThreads = getreg().filter(
			(t: unknown) => typeIs(t, "thread") && coroutine.status(t) !== "dead"
		);
	...
	describe("queueDataUpdate", () => {
		...
		it("should process queue immediately if BATCH_SIZE is reached", () => {
			...
			expect(mockPerson.dataCache).toHaveBeenCalledTimes(math.min(BATCH_SIZE, PerformanceOptimizer.getPerformanceMetrics().maxOperationsPerFrame));
			const metrics = PerformanceOptimizer.getPerformanceMetrics();
			// Queue might not be 0 if BATCH_SIZE > MAX_OPERATIONS_PER_FRAME
			expect(metrics.queueSizes.dataUpdates).toBe(BATCH_SIZE - PerformanceOptimizer.getPerformanceMetrics().maxOperationsPerFrame);
		});
	});

	describe("queueAnalyticsEvent", () => {
		it("should add an analytics event to the queue", () => {
			PerformanceOptimizer.queueAnalyticsEvent({ eventType: "Test", playerId: "1", timestamp: 0, customData: {} });
			const metrics = PerformanceOptimizer.getPerformanceMetrics();
		...
		it("should process analytics queue immediately if BATCH_SIZE is reached", () => {
			...
			for (let i = 0; i < BATCH_SIZE; i++) {
				PerformanceOptimizer.queueAnalyticsEvent({ eventType: "TestBatch", playerId: `${i}`, timestamp: 0, customData: {} });
			}
	...
	describe("queueEffectUpdate", () => {
		...
		it("should process effect update queue immediately if BATCH_SIZE is reached", () => {
			...
			expect(mockPerson.dataCache).toHaveBeenCalledTimes(math.min(BATCH_SIZE, PerformanceOptimizer.getPerformanceMetrics().maxOperationsPerFrame));
			const metrics = PerformanceOptimizer.getPerformanceMetrics();
			expect(metrics.queueSizes.effectUpdates).toBe(BATCH_SIZE - PerformanceOptimizer.getPerformanceMetrics().maxOperationsPerFrame);
		});
	});
