...
describe("Mounts", () => {
	...
	describe("equipMount", () => {
		...
		it("should equip a mount if owned and valid", () => {
			...
			expect((mockPlayer.Character?.FindFirstChild("MountObject") as ObjectValue)?.Value?.Name).toBe("DragonMountModel");
	...
	describe("equipMountFromData", () => {
		...
		it("should do nothing if no mount is saved in data", () => {
			...
			Mounts.equipMountFromData(mockPersonInstance as any);
			expect(mockPlayer.Character?.FindFirstChild("MountObject")?.IsA("ObjectValue") ?
				(mockPlayer.Character?.FindFirstChild("MountObject") as ObjectValue).Value : undefined
			).toBeUndefined();
		});
	});
