...
describe("AdventCalendar", () => {
	...
	it("isAdventCalendarActive should reflect config and current day", () => {
		...
		const configStartTime = os.time({ year: mockConfig.startYear, month: mockConfig.startMonth, day: mockConfig.startDay });
	...
	it("getTimeUntilNextDay should calculate correctly", () => {
		...
		const now = os.time({ year: mockConfig.startYear, month: mockConfig.startMonth, day: mockConfig.startDay, hour: 10, min: 0, sec: 0 });
		...
		if (nextUnlockHour <= 10) { // Next unlock is "tomorrow"
			expectedSecondsUntilNextDay = (24 - 10 + nextUnlockHour) * 3600;
		} else { // Next unlock is "today"
