...
jest.mock("../../../shared/registry/products", () => ({
	...
	1001: {
		Id: 1001, Name: "Small Gold Pack", Price: 100, Type: "Currency",
		Rewards: { Currencies: { Gold: 1000 } },
	},
	1002: {
		Id: 1002, Name: "XP Boost Item", Price: 50, Type: "Item",
		Rewards: { Items: { "XP_BOOST_ITEM_ID": 1 } },
	},
	2001: { // Gamepass example (though gamepass logic is simplified in handler)
		Id: 2001, Name: "VIP Gamepass", Price: 500, Type: "Gamepass",
...
describe("MonetizationHandler", () => {
	...
	it("should initialize and set ProcessReceipt", () => {
		...
		if (MonetizationHandler.processProductPurchase) { // if start() is not the one setting it
	...
	describe("processProductPurchase", () => {
		...
		it("should return PurchaseGranted if purchaseId was already processed", () => {
			...
			expect(rawMockMonetizationData.Currencies.Gold).toBe(ProductsRegistry[receiptInfo.ProductId].Rewards.Currencies.Gold);
		});
